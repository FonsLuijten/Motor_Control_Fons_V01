...
bool QuadEncoder::indexfound()
{
    return ((channel[_encoder_ch].ENC->CTRL & ENC_CTRL_XIRQ_MASK) == ENC_CTRL_XIRQ_MASK);
}
...
{ 
    ...
    channel[index].ENC->CTRL = channel[index].ENC->CTRL & ~ENC_CTRL_XIRQ_MASK & ~ENC_CTRL_XIP_MASK & ~ENC_CTRL_XIE_MASK; // Added by Elwin. This makes index only trigger once
