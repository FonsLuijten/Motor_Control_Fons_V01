...
MotionProfile2::MotionProfile2(float tstart, float t1, float t2, float t3, float p, float v_max, float a_max, float j_max, float Ts ){
    this->tstart = tstart;
    this->t1 = t1;
    this->t2 = t2;
    this->t3 = t3;
...
float MotionProfile2::p2p_get_b( float t ){
    if (t <= REFt0) {
        aref = 0;
        vref = 0;
        qref = REFs0;
...
float MotionProfile2::stateCalculation( int input_status) {
